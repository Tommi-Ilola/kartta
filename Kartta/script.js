...
function searchByKmAndAddMarker(ratakmValue) {
  const url = `https://rata.digitraffic.fi/infra-api/0.7/12680/radat.geojson?time=2023-10-27T09:58:00Z/2023-10-27T09:58:00Z`;
  ...
  fetch(url)
    ...
    .then(data => {
      // Tässä kohtaa sinulla on JSON-data
      // Oletetaan, että haluat käyttää ensimmäisen tuloksen koordinaatteja
      const coordinates = data.features[0].geometry.coordinates;
      // Muunna koordinaatit käyttäen Proj4js-kirjastoa
      const transformedCoordinates = proj4('EPSG:3067', 'EPSG:4326', [coordinates[0], coordinates[1]]);
      // Lisää merkki kartalle
      addMarkerToMap(transformedCoordinates, `Ratakm: ${ratakmValue}`);
    })
    .catch(error => {
...
// Tämä funktio lisää merkin kartalle
function addMarkerToMap(coordinates, title) {
  const marker = L.marker([coordinates[1], coordinates[0]]).bindPopup(title);
  marker.addTo(map);
}

// Käyttäjän syöttämän ratakm-arvon hakeminen ja käsittely
document.getElementById('searchButton').addEventListener('click', function() {
  const searchValue = parseRatakmValue(document.getElementById('searchInput').value);
  searchByKmAndAddMarker(searchValue);
});


var geojsonLayer;

// Lataa ja lisää rautatieliikennepaikat kartalle
fetch('https://rata.digitraffic.fi/infra-api/0.7/12815/rautatieliikennepaikat.geojson?time=2023-11-22T08:30:00Z/2023-11-22T08:30:00Z&typeNames=liikennepaikka')
    .then(response => response.json())
    .then(data => {
    ...
    .catch(error => {
        console.error("Virhe ladattaessa rautatieliikennepaikkojen geometriaa:", error);
    });
